...
import https.siat_impuestos_gob_bo.facturaelectronicaprevalorada.ServicioFacturacionService;
...
public class BillingApiControllerImp implements	
	...
	public CompletionStage<ModelApiResponse> newBilling(Billing body)
			throws Exception {
		return supplyAsync(() -> sendPrevaloredBilling(body), ec);
	}
	 ...
	 */
	private ModelApiResponse sendPrevaloredBilling(Billing body) {
		ModelApiResponse response = new ModelApiResponse();
				try {
					    try {
					    	https.siat_impuestos_gob_bo.facturaelectronicaprevalorada.ServicioFacturacionService service =new ServicioFacturacionService();
						    	...
						    	if (UtilitaryClass.isConnectionVerify(responseVerify)) {
						    		SolicitudRecepcion requestPrevaloredBilling =  new SolicitudRecepcion();
						    		requestPrevaloredBilling  = makeObjectParams(body);
						    		Respuesta responseService = service.getServicioFacturacionPort().recepcionFacturaElectronicaPrevalorada(requestPrevaloredBilling);
	...
	private SolicitudRecepcion makeObjectParams(Billing body) throws DatatypeConfigurationException {
		...
		response.setCuape(body.getCuape());
		return response;
	}
