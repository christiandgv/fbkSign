...
import ec.com.kruger.business.impl.BillingApiControllerImp;
import ec.com.kruger.database.CouchBase;
import ec.com.kruger.database.CouchBaseImp;
...
public class Module extends AbstractModule {
	...
	public void configure() {
		...
		bind(CouchBase.class).to(CouchBaseImp.class).asEagerSingleton();
		bind(BillingApiControllerImpInterface.class).to(BillingApiControllerImp.class).asEagerSingleton();
...
class MetricRegistryProvider implements Provider<MetricRegistry> {
	...
	private void slf4jReporter() {
		final Slf4jReporter reporter = Slf4jReporter.forRegistry(registry).outputTo(logger).convertRatesTo(TimeUnit.SECONDS).convertDurationsTo(TimeUnit.MILLISECONDS).build();
